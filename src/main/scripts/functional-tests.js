...
import { HTTP_STATUS, THRESHOLDS } from "../common/constants.js";
...
function testValidOrder(orderRequest, testName) {
  ...
  if (response.status === HTTP_STATUS.OK) {
    try {
      ...
      check(orderResponse, {
        [`${testName}: has pizza object`]: (r) =>
          r.pizza !== undefined && typeof r.pizza === "object",
        [`${testName}: has calories`]: (r) =>
          typeof r.calories === "number" && r.calories > 0,
        [`${testName}: has vegetarian flag`]: (r) =>
      ...
      if (orderRequest.mustBeVegetarian) {
        check(orderResponse, {
          [`${testName}: vegetarian requirement met`]: (r) =>
            r.vegetarian === true,
        });
      }
      ...
      check(orderResponse, {
        [`${testName}: calories within limit`]: () =>
          caloriesPerSlice <= orderRequest.maxCaloriesPerSlice,
      });

      // Ingredient exclusions validation
      if (orderRequest.excludedIngredients.length > 0) {
        const ingredientNames = orderResponse.pizza.ingredients.map(
          (i) => i.name
        );
        const hasExcludedIngredient = orderRequest.excludedIngredients.some(
          (excluded) => ingredientNames.includes(excluded)
        );

        check(orderResponse, {
          [`${testName}: excluded ingredients not present`]: () =>
...
function testInvalidOrder(orderRequest, testName) {
  ...
  check(response, {
    [`${testName}: returns error status`]: (r) =>
...
function testSpecialCharacters() {
  ...
  check(response, {
    "Special chars: handled correctly": (r) =>

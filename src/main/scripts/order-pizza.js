...
function orderPizzaTest(orderRequest, testType) {
  ...
  if (isValidRequest) {
    ...
    if (response.status === HTTP_STATUS.OK) {
      try {
        ...
        check(orderResponse, {
          ...
          "response has calories": (r) =>
            r.calories !== undefined && r.calories > 0,
          "response has vegetarian flag": (r) =>
            typeof r.vegetarian === "boolean",
          "pizza has valid ID": (r) => r.pizza && r.pizza.id > 0,
          "pizza has name": (r) =>
            r.pizza && r.pizza.name !== undefined && r.pizza.name.length > 0,
          "pizza has dough": (r) => r.pizza && r.pizza.dough !== undefined,
          "pizza has ingredients": (r) =>
            r.pizza && Array.isArray(r.pizza.ingredients),
        });

        // Validate business rules
        if (orderRequest.mustBeVegetarian) {
          check(orderResponse, {
            "vegetarian order returns vegetarian pizza": (r) =>
              r.vegetarian === true,
          });
        }
    ...
    check(response, {
      "status is 4xx for invalid request": (r) =>

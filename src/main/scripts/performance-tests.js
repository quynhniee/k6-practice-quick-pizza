...
import { generateOrderPizzaRequest } from "../utils/generate-randomly-data.js";
...
export function setup() {
  ...
  console.log(
    `Test scenarios: ${Object.keys(options.scenarios || {}).join(", ")}`
  );
...
function performanceOrderTest(orderRequest, testType) {
  ...
  const performanceChecks = {
    ...
    "response time acceptable": (r) => {
      ...
      switch (testType) {
        ...
        case "peak":
          return r.timings.duration < 2500;
        case "stress":
          return r.timings.duration < 4000;
        case "spike":
          return r.timings.duration < 6000;
        default:
    ...
    check(response, {
      "valid response structure": (r) => {
        try {
          ...
          return (
            data.pizza && data.calories && typeof data.vegetarian === "boolean"
          );
        } catch {
          return false;
        }
  ...
  if (response.timings.duration > 3000) {
    console.warn(
      `Slow response: ${response.timings.duration}ms for ${testType} (VU: ${__VU}, Iter: ${__ITER})`
    );
  }

  if (response.status >= 500) {
    console.error(
      `Server error: ${response.status} for ${testType} (VU: ${__VU}, Iter: ${__ITER})`
    );
  }
}

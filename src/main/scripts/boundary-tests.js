  ...
  group("Calorie Boundary Tests", () => {
    ...
    boundaryTests.forEach((test) => {
      ...
      const response = makeRequest(
        orderRequest,
        `calorie-boundary-${test.name}`
      );

      if (test.calories <= 0) {
        check(response, {
          [`${test.name}: should reject non-positive calories`]: (r) =>
            r.status >= 400,
        });
      } else if (test.calories >= 1000) {
        check(response, {
          [`${test.name}: should handle high calorie requests`]: (r) =>
            r.status === 200 || r.status >= 400,
        });
      } else {
        check(response, {
          [`${test.name}: should accept valid calories`]: (r) =>
            r.status === 200,
        });
      }
    });
...
function testToppingBoundaries() {
  group("Topping Count Boundary Tests", () => {
    ...
    toppingTests.forEach((test) => {
      ...
      const response = makeRequest(
        orderRequest,
        `topping-boundary-${test.name}`
      );

      if (test.min > test.max || test.min < 0 || test.max < 0) {
        check(response, {
          [`${test.name}: should reject invalid topping range`]: (r) =>
            r.status >= 400,
        });
      } else {
        check(response, {
          [`${test.name}: should handle valid topping range`]: (r) =>
            r.status === 200 || r.status >= 400,
        });
      }
    });
  ...
  group("Name Length Boundary Tests", () => {
    ...
    nameTests.forEach((test) => {
      ...
      if (test.value === "" || test.value.trim() === "") {
        check(response, {
          [`${test.name}: should reject empty/whitespace names`]: (r) =>
            r.status >= 400,
        });
      } else if (test.value.length > 500) {
        check(response, {
          [`${test.name}: should handle very long names`]: (r) =>
            r.status === 200 || r.status >= 400,
        });
      } else {
        check(response, {
...
function testEmptyAndNullValues() {
  group("Empty and Null Value Tests", () => {
    ...
    emptyTests.forEach((test) => {
      ...
      check(response, {
        [`${test.name}: handles empty/large arrays`]: (r) =>
          r.status === 200 || r.status >= 400,
      });
    });
  });
...
function testExtremeValues() {
  group("Extreme Value Tests", () => {
    ...
    extremeTests.forEach((test) => {
      ...
      check(response, {
        [`${test.name}: handles extreme values gracefully`]: (r) =>
          r.status !== undefined && r.timings.duration < 10000,
      });
    });
  });
}
...
function testSpecialCharacterHandling() {
  group("Special Character Tests", () => {
    ...
    specialCharTests.forEach((test) => {
      ...
      check(response, {
        [`${test.name}: handles special characters safely`]: (r) =>
          r.status !== undefined,
...
function testMalformedRequests() {
  group("Malformed Request Tests", () => {
    ...
    malformedTests.forEach((test) => {
      const response = http.post(url, test.body, {
        headers,
        tags: { test_case: test.name },
      });
      check(response, {
        [`${test.name}: rejects malformed request`]: (r) => r.status >= 400,
        [`${test.name}: quick response to malformed data`]: (r) =>
...
function testUnicodeHandling() {
  group("Unicode Handling Tests", () => {
    ...
    unicodeTests.forEach((test) => {
      ...
      check(response, {
        [`${test.name}: handles unicode correctly`]: (r) =>
          r.status !== undefined,
      });
    });
  });
